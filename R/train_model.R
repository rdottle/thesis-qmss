...
census = readOGR("../Desktop/Git/thesis-qmss/data/", layer="chicaco_census")
...
centrality = read_csv("../Desktop/Git/thesis-qmss/data/chicago_networkfeatures.csv")
...
nb <- poly2nb(census, census@data$density)
weights_list <- nb2listw(nb, style = "W", zero.policy = TRUE)
print(weights_list, zero.policy=T)
W = as.matrix(weights_list)
ptrW = trW(W)
nb
census@data$pov_lag = lag.listw(weights_list, as.numeric(census@data$density), zero.policy = T)
lag.plot(as.numeric(census$density), lags =1)
pov_lag = as.data.frame(census$pov_lag)

length(weights_list$)
census$GEOID
centrality$GEOID
data = census@data %>% dplyr::select(GEOID, density, closeness, betweenness, per_pover2)

row.names(data) = data$GEOID
data[is.na(data)] = 0 
data %<>% dplyr::select(density:betweenness)
...
glm.fit = train(per_pover2 ~ ., data = train,
                method = "glmnet",
